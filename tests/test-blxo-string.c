...
test_str_elide_underscores (void)
{
  ...
  res = blxo_str_elide_underscores ("this_is_a_sample");
  g_assert_cmpstr (res, ==, "thisisasample");
  g_free (res);

  res = blxo_str_elide_underscores ("m__nemonic");
...
test_str_is_equal (void)
{
  ...
  g_assert (!blxo_str_is_equal ("a", NULL));
  g_assert (!blxo_str_is_equal (NULL, "b"));
  g_assert (!blxo_str_is_equal ("a", "abcde"));
  g_assert (!blxo_str_is_equal (p, "a"));

  /* comparison that should return TRUE */
  g_assert (blxo_str_is_equal (NULL, NULL));
  g_assert (blxo_str_is_equal ("test", "test"));
  g_assert (blxo_str_is_equal (p, p));
}
...
test_str_replace (void)
{
  ...
  res = blxo_str_replace (test, "fruits", "pizza");
  g_assert_cmpstr (res, ==, "You should eat pizza every day.");
  g_free (res);

  res = blxo_str_replace (test, " fruits every day", NULL);
  g_assert_cmpstr (res, ==, "You should eat.");
  g_free (res);

  res = blxo_str_replace (NULL, NULL, NULL);
  g_assert (res == NULL);

  res = blxo_str_replace (test, NULL, NULL);
