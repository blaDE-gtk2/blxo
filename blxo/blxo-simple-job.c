...
/**
 * SECTION: blxo-simple-job
 * @title: BlxoSimpleJob
 * @short_description: Simple interface to execute functions asynchronously
 * @include: blxo/blxo.h
 * @see_also: <link linkend="BlxoJob">BlxoJob</link>
 *
 * <link linkend="BlxoSimpleJob">BlxoSimpleJob</link> can be used to execute
 * functions asynchronously in an #BlxoJob wrapper object. It is easier to
...
static void     blxo_simple_job_finalize   (GObject           *object);
static gboolean blxo_simple_job_execute    (BlxoJob            *job,
                                           GError           **error);



struct _BlxoSimpleJobClass
{
 ...
 * BlxoSimpleJob:
 *
 * The #BlxoSimpleJob struct contains only private fields and should not be
 * directly accessed.
 **/
struct _BlxoSimpleJob
{
  BlxoSimpleJobFunc func;
...
G_DEFINE_TYPE (BlxoSimpleJob, blxo_simple_job, BLXO_TYPE_JOB)



static void
blxo_simple_job_class_init (BlxoSimpleJobClass *klass)
{
  BlxoJobClass  *blxojob_class;
  GObjectClass *gobject_class;

  gobject_class = G_OBJECT_CLASS (klass);
  gobject_class->finalize = blxo_simple_job_finalize;

  blxojob_class = BLXO_JOB_CLASS (klass);
  blxojob_class->execute = blxo_simple_job_execute;
}



static void
blxo_simple_job_init (BlxoSimpleJob *simple_job)
{
}
...
static void
blxo_simple_job_finalize (GObject *object)
{
  BlxoSimpleJob *simple_job = BLXO_SIMPLE_JOB (object);

  /* release the param values */
  g_value_array_free (simple_job->param_values);

  (*G_OBJECT_CLASS (blxo_simple_job_parent_class)->finalize) (object);
}



static gboolean
blxo_simple_job_execute (BlxoJob  *job,
                        GError **error)
{
  BlxoSimpleJob *simple_job = BLXO_SIMPLE_JOB (job);
  gboolean         success = TRUE;
  GError          *err = NULL;

  _blxo_return_val_if_fail (BLXO_IS_SIMPLE_JOB (job), FALSE);
  _blxo_return_val_if_fail (simple_job->func != NULL, FALSE);
 ...
 * @func           : the #BlxoSimpleJobFunc to execute the job.
 ...
 *
 * Allocates a new #BlxoJob which executes the specified @func with
 ...
 *         break;
 *
 *       blxo_job_info_message (job, _("Child: %s"),
 ...
 * BlxoJob *job = blxo_simple_job_launch (list_directory, 1, G_TYPE_FILE, file);
 ...
 *
 * The caller is responsible to release the returned #BlxoJob object
 * using g_object_unref() when no longer needed.
 *
 * Returns: the launched #BlxoJob.
 **/
BlxoJob*
blxo_simple_job_launch (BlxoSimpleJobFunc func,
                       guint            n_param_values,
                       ...)
{
  BlxoSimpleJob *simple_job;
  va_list       var_args;
  GValue        value = { 0, };
  gchar        *error_message;
  guint         n;
  ...
  simple_job = g_object_new (BLXO_TYPE_SIMPLE_JOB, NULL);
  ...
  /* launch the job */
  return blxo_job_launch (BLXO_JOB (simple_job));
}


#define __BLXO_SIMPLE_JOB_C__
