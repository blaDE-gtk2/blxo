 ...
 **/
gchar*
blxo_str_elide_underscores (const gchar *text)
{
  const gchar *s;
 ...
 **/
gboolean
blxo_str_is_equal (const gchar *a,
                  const gchar *b)
{
  if (a == NULL || b == NULL)
    return (a == b);
 ...
 **/
gchar*
blxo_str_replace (const gchar *str,
                 const gchar *pattern,
      ...
      if (G_UNLIKELY (*str == *pattern))
        {
          ...
          if (G_LIKELY (*p == '\0'))
            {
              if (G_LIKELY (!blxo_str_is_empty (replacement)))
 ...
 **/
gchar*
blxo_strdup_strftime (const gchar     *format,
                     const struct tm *tm)
{
  ...
  for (;;)
    {
      ...
      switch (*remainder)
        ...
        case '\0':
          g_warning ("Trailing %% passed to blxo_strdup_strftime");
          g_string_append_c (string, '%');
          continue;
      ...
      if (strchr (SUS_EXTENDED_STRFTIME_MODIFIERS, *remainder) != NULL)
        {
          ...
          if (*remainder == 0)
            {
              g_warning ("Unfinished %%%c modifier passed to blxo_strdup_strftime", modifier);
              break;
            }
        }

      if (strchr (C_STANDARD_STRFTIME_CHARACTERS, *remainder) == NULL)
        g_warning ("blxo_strdup_strftime does not support non-standard escape code %%%c", *remainder);
      ...
      if (strip_leading_zeros || turn_leading_zeros_to_spaces)
        {
          if (strchr (C_STANDARD_NUMERIC_STRFTIME_CHARACTERS, *remainder) == NULL)
            g_warning ("blxo_strdup_strftime does not support modifier for non-numeric escape code %%%c%c", remainder[-1], *remainder);
 ...
 * of @strv. If called on a %NULL value or @num is 0, blxo_strndupv()
 * simply returns %NULL.
 *
 ...
 **/
gboolean
blxo_str_looks_like_an_uri (const gchar *str)
{
  const gchar *s = str;
 ...
 **/
gboolean
blxo_str_is_flag (const gchar *str)
{
  if (G_UNLIKELY (str == NULL))
    return FALSE;
