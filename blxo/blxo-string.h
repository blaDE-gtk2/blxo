...
#ifndef __BLXO_STRING_H__
#define __BLXO_STRING_H__

#include <blxo/blxo-config.h>

G_BEGIN_DECLS

gchar                *blxo_str_elide_underscores  (const gchar     *text) G_GNUC_MALLOC G_GNUC_WARN_UNUSED_RESULT;

gboolean              blxo_str_is_equal           (const gchar     *a,
                                                  const gchar     *b);

gchar                *blxo_str_replace            (const gchar     *str,
                                                  const gchar     *pattern,
                                                  const gchar     *replacement) G_GNUC_MALLOC G_GNUC_WARN_UNUSED_RESULT;

gchar                *blxo_strdup_strftime        (const gchar     *format,
                                                  const struct tm *tm) G_GNUC_MALLOC G_GNUC_WARN_UNUSED_RESULT;

gchar               **blxo_strndupv               (gchar          **strv,
                                                  guint            num) G_GNUC_MALLOC G_GNUC_WARN_UNUSED_RESULT;

gboolean              blxo_str_looks_like_an_uri  (const gchar     *str);

gboolean              blxo_str_is_flag            (const gchar     *str);
 ...
 * Since : 0.5.0
 **/
#define blxo_str_is_empty(string) ((string) == NULL || *(string) == '\0')
