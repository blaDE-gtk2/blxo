...
/**
 * SECTION: blxo-tree-view
 * @title: BlxoTreeView
 * @short_description: An improved version of #GtkTreeView
 * @include: blxo/blxo.h
 *
 * The #BlxoTreeView class derives from #GtkTreeView and extends it with
 ...
 * of #GtkTreeView, i.e. #BlxoTreeView allows the user to drag around multiple
 * selected rows.
 **/
...
static void     blxo_tree_view_finalize                      (GObject          *object);
static void     blxo_tree_view_get_property                  (GObject          *object,
                                                             guint             prop_id,
                                                             GValue           *value,
                                                             GParamSpec       *pspec);
static void     blxo_tree_view_set_property                  (GObject          *object,
                                                             guint             prop_id,
                                                             const GValue     *value,
                                                             GParamSpec       *pspec);
static gboolean blxo_tree_view_button_press_event            (GtkWidget        *widget,
                                                             GdkEventButton   *event);
static gboolean blxo_tree_view_button_release_event          (GtkWidget        *widget,
                                                             GdkEventButton   *event);
static gboolean blxo_tree_view_motion_notify_event           (GtkWidget        *widget,
                                                             GdkEventMotion   *event);
static gboolean blxo_tree_view_leave_notify_event            (GtkWidget        *widget,
                                                             GdkEventCrossing *event);
static void     blxo_tree_view_drag_begin                    (GtkWidget        *widget,
                                                             GdkDragContext   *context);
static gboolean blxo_tree_view_move_cursor                   (GtkTreeView      *view,
                                                             GtkMovementStep   step,
                                                             gint              count);
static gboolean blxo_tree_view_single_click_timeout          (gpointer          user_data);
static void     blxo_tree_view_single_click_timeout_destroy  (gpointer          user_data);



struct _BlxoTreeViewPrivate
{
...
G_DEFINE_TYPE_WITH_PRIVATE (BlxoTreeView, blxo_tree_view, GTK_TYPE_TREE_VIEW)



static void
blxo_tree_view_class_init (BlxoTreeViewClass *klass)
{
  ...
  gobject_class->finalize = blxo_tree_view_finalize;
  gobject_class->get_property = blxo_tree_view_get_property;
  gobject_class->set_property = blxo_tree_view_set_property;

  gtkwidget_class = GTK_WIDGET_CLASS (klass);
  gtkwidget_class->button_press_event = blxo_tree_view_button_press_event;
  gtkwidget_class->button_release_event = blxo_tree_view_button_release_event;
  gtkwidget_class->motion_notify_event = blxo_tree_view_motion_notify_event;
  gtkwidget_class->leave_notify_event = blxo_tree_view_leave_notify_event;
  gtkwidget_class->drag_begin = blxo_tree_view_drag_begin;

  gtktree_view_class = GTK_TREE_VIEW_CLASS (klass);
  gtktree_view_class->move_cursor = blxo_tree_view_move_cursor;

  /* initialize the library's i18n support */
  _blxo_i18n_init ();

  /**
   * BlxoTreeView:single-click:
   *
  ...
  /**
   * BlxoTreeView:single-click-timeout:
   *
...
static void
blxo_tree_view_init (BlxoTreeView *tree_view)
{
  /* grab a pointer on the private data */
  tree_view->priv = blxo_tree_view_get_instance_private (tree_view);
...
static void
blxo_tree_view_finalize (GObject *object)
{
  BlxoTreeView *tree_view = BLXO_TREE_VIEW (object);
  ...
  (*G_OBJECT_CLASS (blxo_tree_view_parent_class)->finalize) (object);
}



static void
blxo_tree_view_get_property (GObject    *object,
                            guint       prop_id,
                            GValue     *value,
                            GParamSpec *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_SINGLE_CLICK:
      g_value_set_boolean (value, blxo_tree_view_get_single_click (tree_view));
      break;

    case PROP_SINGLE_CLICK_TIMEOUT:
      g_value_set_uint (value, blxo_tree_view_get_single_click_timeout (tree_view));
      break;
...
static void
blxo_tree_view_set_property (GObject      *object,
                            guint         prop_id,
                            const GValue *value,
                            GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_SINGLE_CLICK:
      blxo_tree_view_set_single_click (tree_view, g_value_get_boolean (value));
      break;

    case PROP_SINGLE_CLICK_TIMEOUT:
      blxo_tree_view_set_single_click_timeout (tree_view, g_value_get_uint (value));
      break;
...
static gboolean
blxo_tree_view_button_press_event (GtkWidget      *widget,
                                  GdkEventButton *event)
{
  GtkTreeSelection *selection;
  BlxoTreeView      *tree_view = BLXO_TREE_VIEW (widget);
  ...
  if (event->type == GDK_BUTTON_PRESS && (event->state & gtk_accelerator_get_default_mod_mask ()) == 0
    ...
    {
      /* if no custom select function is set, we simply use blxo_noop_false here,
      ...
      if (G_LIKELY (gtk_tree_selection_get_select_function (selection) == NULL))
        gtk_tree_selection_set_select_function (selection, (GtkTreeSelectionFunc) (void (*)(void)) blxo_noop_false, NULL, NULL);
      else
  ...
  result = (*GTK_WIDGET_CLASS (blxo_tree_view_parent_class)->button_press_event) (widget, event);
  ...
  if (event->type == GDK_BUTTON_PRESS && (event->state & gtk_accelerator_get_default_mod_mask ()) == 0
      && path != NULL && gtk_tree_selection_path_is_selected (selection, path))
    {
      ...
      if (G_LIKELY (gtk_tree_selection_get_select_function (selection) != (GtkTreeSelectionFunc) (void (*)(void)) blxo_noop_false))
        {
  ...
  if (G_LIKELY (gtk_tree_selection_get_select_function (selection) == (GtkTreeSelectionFunc) (void (*)(void)) blxo_noop_false))
    {
      gtk_tree_selection_set_select_function (selection, (GtkTreeSelectionFunc) (void (*)(void)) blxo_noop_true, NULL, NULL);
    }
...
static gboolean
blxo_tree_view_button_release_event (GtkWidget      *widget,
                                    GdkEventButton *event)
{
  ...
  return (*GTK_WIDGET_CLASS (blxo_tree_view_parent_class)->button_release_event) (widget, event);
}



static gboolean
blxo_tree_view_motion_notify_event (GtkWidget      *widget,
                                   GdkEventMotion *event)
{
  BlxoTreeView *tree_view = BLXO_TREE_VIEW (widget);
  GtkTreePath *path;
              ...
              if (G_LIKELY (tree_view->priv->single_click_timeout > 0 && tree_view->priv->hover_path != NULL))
                {
                  ...
                  tree_view->priv->single_click_timeout_id = gdk_threads_add_timeout_full (G_PRIORITY_LOW, tree_view->priv->single_click_timeout,
                                                                                           blxo_tree_view_single_click_timeout, tree_view,
                                                                                           blxo_tree_view_single_click_timeout_destroy);
                }
            }
          else
            {
  ...
  return (*GTK_WIDGET_CLASS (blxo_tree_view_parent_class)->motion_notify_event) (widget, event);
}



static gboolean
blxo_tree_view_leave_notify_event (GtkWidget        *widget,
                                  GdkEventCrossing *event)
{
  BlxoTreeView *tree_view = BLXO_TREE_VIEW (widget);
  ...
  return (*GTK_WIDGET_CLASS (blxo_tree_view_parent_class)->leave_notify_event) (widget, event);
}



static void
blxo_tree_view_drag_begin (GtkWidget      *widget,
                          GdkDragContext *context)
{
  BlxoTreeView *tree_view = BLXO_TREE_VIEW (widget);
  ...
  (*GTK_WIDGET_CLASS (blxo_tree_view_parent_class)->drag_begin) (widget, context);
}



static gboolean
blxo_tree_view_move_cursor (GtkTreeView    *view,
                           GtkMovementStep step,
                           gint            count)
{
  BlxoTreeView *tree_view = BLXO_TREE_VIEW (view);
  ...
  return (*GTK_TREE_VIEW_CLASS (blxo_tree_view_parent_class)->move_cursor) (view, step, count);
}



static gboolean
blxo_tree_view_single_click_timeout (gpointer user_data)
{
  ...
  GtkTreeIter        iter;
  BlxoTreeView       *tree_view = BLXO_TREE_VIEW (user_data);
...
static void
blxo_tree_view_single_click_timeout_destroy (gpointer user_data)
{
  BLXO_TREE_VIEW (user_data)->priv->single_click_timeout_id = -1;
}
 ...
 * blxo_tree_view_new:
 *
 * Allocates a new #BlxoTreeView instance.
 *
 * Returns: the newly allocated #BlxoTreeView.
 *
 * Since: 0.3.1.3
 **/
GtkWidget*
blxo_tree_view_new (void)
{
  return g_object_new (BLXO_TYPE_TREE_VIEW, NULL);
}
 ...
 **/
gboolean
blxo_tree_view_get_single_click (const BlxoTreeView *tree_view)
{
  g_return_val_if_fail (BLXO_IS_TREE_VIEW (tree_view), FALSE);
 ...
 **/
void
blxo_tree_view_set_single_click (BlxoTreeView *tree_view,
                                gboolean     single_click)
{
  g_return_if_fail (BLXO_IS_TREE_VIEW (tree_view));
 ...
 **/
guint
blxo_tree_view_get_single_click_timeout (const BlxoTreeView *tree_view)
{
  g_return_val_if_fail (BLXO_IS_TREE_VIEW (tree_view), 0u);
 ...
 * single-click mode, see blxo_tree_view_set_single_click().
 *
 * Since: 0.3.1.5
 **/
void
blxo_tree_view_set_single_click_timeout (BlxoTreeView *tree_view,
                                        guint        single_click_timeout)
{
  g_return_if_fail (BLXO_IS_TREE_VIEW (tree_view));
