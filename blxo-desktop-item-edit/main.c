...
#include <blxo-desktop-item-edit/blxo-die-utils.h>
...
static void blxo_die_error (const gchar *format, ...) G_GNUC_PRINTF (1, 2);
...
static void
blxo_die_error (const gchar *format,
               ...)
{
  gchar   *buffer;
  va_list  args;
...
static void
blxo_die_help (GtkWidget *button,
              GtkWidget *dialog)
{
  xfce_dialog_show_help (GTK_WINDOW (dialog), "blxo", "desktop-item-edit", NULL);
}
...
main (int argc, char **argv)
{
      ...
      if (G_UNLIKELY (error != NULL))
        {
          ...
          blxo_die_error ("%s", error->message);
          g_error_free (error);
        }
      else
        {
          /* no error message, the GUI initialization failed */
          blxo_die_error ("%s %s", _("Failed to open display"),
  ...
  if (G_UNLIKELY (argc != 2))
    {
      blxo_die_error (_("No file/folder specified"));
      return EXIT_FAILURE;
    }
  ...
  if (opt_create_new)
    {
      ...
      /* type specific stuff */
      if (blxo_str_is_equal (opt_type, G_KEY_FILE_DESKTOP_TYPE_LINK))
        {
          g_key_file_set_value (key_file, G_KEY_FILE_DESKTOP_GROUP,
      ...
      else if (blxo_str_is_equal (opt_type, G_KEY_FILE_DESKTOP_TYPE_DIRECTORY))
        {
          g_key_file_set_value (key_file, G_KEY_FILE_DESKTOP_GROUP,
                                G_KEY_FILE_DESKTOP_KEY_ICON, STR_FB (opt_icon, ""));
        }
      else if (blxo_str_is_equal (opt_type, G_KEY_FILE_DESKTOP_TYPE_APPLICATION))
        {
          g_key_file_set_value (key_file, G_KEY_FILE_DESKTOP_GROUP,
      ...
      if (G_UNLIKELY (!res || length == 0))
        {
          ...
          if (G_LIKELY (error != NULL))
            {
              blxo_die_error (_("Failed to load contents from \"%s\": %s"), argv[1], error->message);
              g_error_free (error);
            }
          else
            {
              blxo_die_error (_("The file \"%s\" contains no data"), argv[1]);
            }
      ...
      if (G_UNLIKELY (!res))
        {
          ...
          blxo_die_error (_("Failed to parse contents of \"%s\": %s"), argv[1], error->message);
          g_error_free (error);
          return EXIT_FAILURE;
        }
    }
  ...
  if (G_UNLIKELY (value == NULL))
    {
      ...
      blxo_die_error (_("File \"%s\" has no type key"), argv[1]);
      return EXIT_FAILURE;
    }

  /* verify that we support the type */
  enum_klass = g_type_class_ref (BLXO_DIE_TYPE_EDITOR_MODE);
  enum_value = g_enum_get_value_by_nick (enum_klass, value);
  if (G_UNLIKELY (enum_value == NULL))
    {
      /* tell the user that we don't support the type */
      blxo_die_error (_("Unsupported desktop file type \"%s\""), value);
      return EXIT_FAILURE;
    }
  g_free (value);
  ...
  blxo_gtk_dialog_add_secondary_button(GTK_DIALOG (dialog), GTK_WIDGET (button));
  g_signal_connect (G_OBJECT (button), "clicked",
      G_CALLBACK (blxo_die_help), dialog);
  ...
  editor = blxo_die_editor_new ();
  blxo_die_editor_set_mode (BLXO_DIE_EDITOR (editor), mode);
  gtk_container_set_border_width (GTK_CONTAINER (editor), 12);
  blxo_binding_new (G_OBJECT (editor), "complete", G_OBJECT (button), "sensitive");
  ...
  blxo_die_editor_set_name (BLXO_DIE_EDITOR (editor), (value != NULL) ? value : "");
  g_free (value);

  /* setup the comment */
  value = g_key_file_get_locale_string (key_file, G_KEY_FILE_DESKTOP_GROUP,
                                        G_KEY_FILE_DESKTOP_KEY_COMMENT, NULL, NULL);
  blxo_die_editor_set_comment (BLXO_DIE_EDITOR (editor), (value != NULL) ? value : "");
  g_free (value);
  ...
  blxo_die_editor_set_icon (BLXO_DIE_EDITOR (editor), (value != NULL) ? value : "");
  g_free (value);
  ...
  switch (mode)
    ...
    case BLXO_DIE_EDITOR_MODE_APPLICATION:
      ...
      blxo_die_editor_set_command (BLXO_DIE_EDITOR (editor), (value != NULL) ? value : "");
      g_free (value);

      /* setup the working directory */
      value = g_key_file_get_string (key_file, G_KEY_FILE_DESKTOP_GROUP,
                                     G_KEY_FILE_DESKTOP_KEY_PATH, NULL);
      blxo_die_editor_set_path (BLXO_DIE_EDITOR (editor), (value != NULL) ? value : "");
      g_free (value);


      /* setup launcher options */
      blxo_die_editor_set_snotify (BLXO_DIE_EDITOR (editor),
          g_key_file_get_boolean (key_file, G_KEY_FILE_DESKTOP_GROUP,
                                  G_KEY_FILE_DESKTOP_KEY_STARTUP_NOTIFY, NULL));
      blxo_die_editor_set_terminal (BLXO_DIE_EDITOR (editor),
          g_key_file_get_boolean (key_file, G_KEY_FILE_DESKTOP_GROUP,
    ...
    case BLXO_DIE_EDITOR_MODE_LINK:
      ...
      blxo_die_editor_set_url (BLXO_DIE_EDITOR (editor), (value != NULL) ? value : "");
      g_free (value);
      break;
  ...
  if (response == GTK_RESPONSE_ACCEPT)
    {
      ...
      blxo_die_g_key_file_set_locale_value (key_file, G_KEY_FILE_DESKTOP_GROUP,
                                           G_KEY_FILE_DESKTOP_KEY_NAME,
                                           blxo_die_editor_get_name (BLXO_DIE_EDITOR (editor)));
      blxo_die_g_key_file_set_locale_value (key_file, G_KEY_FILE_DESKTOP_GROUP,
                                           G_KEY_FILE_DESKTOP_KEY_ICON,
                                           blxo_die_editor_get_icon (BLXO_DIE_EDITOR (editor)));
      blxo_die_g_key_file_set_locale_value (key_file, G_KEY_FILE_DESKTOP_GROUP,
                                           G_KEY_FILE_DESKTOP_KEY_COMMENT,
                                           blxo_die_editor_get_comment (BLXO_DIE_EDITOR (editor)));
      ...
      switch (mode)
        ...
        case BLXO_DIE_EDITOR_MODE_APPLICATION:
          ...
          g_key_file_set_string (key_file, G_KEY_FILE_DESKTOP_GROUP,
                                 ...
                                 blxo_die_editor_get_command (BLXO_DIE_EDITOR (editor)));

          /* save the new command */
          g_key_file_set_string (key_file, G_KEY_FILE_DESKTOP_GROUP,
                                 G_KEY_FILE_DESKTOP_KEY_PATH,
                                 blxo_die_editor_get_path (BLXO_DIE_EDITOR (editor)));

          /* save the new launcher options */
          g_key_file_set_boolean (key_file, G_KEY_FILE_DESKTOP_GROUP,
                                 G_KEY_FILE_DESKTOP_KEY_TERMINAL,
                                 blxo_die_editor_get_terminal (BLXO_DIE_EDITOR (editor)));
          g_key_file_set_boolean (key_file, G_KEY_FILE_DESKTOP_GROUP,
                                 G_KEY_FILE_DESKTOP_KEY_STARTUP_NOTIFY,
                                 blxo_die_editor_get_snotify (BLXO_DIE_EDITOR (editor)));
          break;
        ...
        case BLXO_DIE_EDITOR_MODE_LINK:
          ...
          g_key_file_set_string (key_file, G_KEY_FILE_DESKTOP_GROUP,
                                 ...
                                 blxo_die_editor_get_url (BLXO_DIE_EDITOR (editor)));
          break;
      ...
      if (!blxo_die_g_key_file_save (key_file, opt_create_new, trusted_launcher, gfile, mode, &error))
        {
          if (opt_create_new)
            {
              /* reset the error */
              ...
              if (G_LIKELY (response == GTK_RESPONSE_ACCEPT))
                {
                  ...
                  blxo_die_g_key_file_save (key_file, FALSE, trusted_launcher, gfile, mode, &error);
                }
            ...
            {
              if (mode == BLXO_DIE_EDITOR_MODE_DIRECTORY)
                  ...
                  if (G_LIKELY (path != NULL))
                    {
                      ...
                      blxo_die_error ("\"%s\" is not writeable, saving to \"%s\" instead.",
                                     argv[1], path);
                      ...
                      blxo_die_g_key_file_save (key_file, FALSE, trusted_launcher, gfile_local, mode, &error);
