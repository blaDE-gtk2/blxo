...
static void blxo_helper_chooser_finalize     (GObject                *object);
static void blxo_helper_chooser_get_property (GObject                *object,
                                             guint                   prop_id,
                                             GValue                 *value,
                                             GParamSpec             *pspec);
static void blxo_helper_chooser_set_property (GObject                *object,
                                             guint                   prop_id,
                                             const GValue           *value,
                                             GParamSpec             *pspec);
static void blxo_helper_chooser_update       (BlxoHelperChooser       *chooser);
static void blxo_helper_chooser_pressed      (BlxoHelperChooser       *chooser,
                                             GtkWidget              *button);



struct _BlxoHelperChooserClass
{
  GtkBinClass __parent__;
};

struct _BlxoHelperChooser
{
  GtkBin __parent__;
...
G_DEFINE_TYPE (BlxoHelperChooser, blxo_helper_chooser, GTK_TYPE_BIN)



static void
blxo_helper_chooser_class_init (BlxoHelperChooserClass *klass)
{
  GObjectClass *gobject_class;

  gobject_class = G_OBJECT_CLASS (klass);
  gobject_class->finalize = blxo_helper_chooser_finalize;
  gobject_class->get_property = blxo_helper_chooser_get_property;
  gobject_class->set_property = blxo_helper_chooser_set_property;

  /**
   * BlxoHelperChooser:category:
   *
   * The #BlxoHelperCategory which should be configured by this
   * #BlxoHelperChooser. See blxo_helper_chooser_get_category() and
   * blxo_helper_chooser_set_category() for details.
   **/
  ...
  /**
   * BlxoHelperChooser:is-valid:
   *
   * %TRUE if a valid #BlxoHelper is selected by
   * this #BlxoHelperChooser, else %FALSE.
   **/
  g_object_class_install_property (gobject_class,
                                   PROP_IS_VALID,
...
static void
blxo_helper_chooser_init (BlxoHelperChooser *chooser)
{
  ...
  chooser->database = blxo_helper_database_get ();

  /*gtk_widget_push_composite_child ();*/

  button = gtk_button_new ();
  g_signal_connect_swapped (G_OBJECT (button), "pressed", G_CALLBACK (blxo_helper_chooser_pressed), chooser);
...
static void
blxo_helper_chooser_finalize (GObject *object)
{
  BlxoHelperChooser *chooser = BLXO_HELPER_CHOOSER (object);

  g_object_unref (G_OBJECT (chooser->database));

  (*G_OBJECT_CLASS (blxo_helper_chooser_parent_class)->finalize) (object);
}



static void
blxo_helper_chooser_get_property (GObject    *object,
                                 guint       prop_id,
                                 GValue     *value,
                                 GParamSpec *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_CATEGORY:
      g_value_set_enum (value, blxo_helper_chooser_get_category (chooser));
      break;

    case PROP_IS_VALID:
      g_value_set_boolean (value, blxo_helper_chooser_get_is_valid (chooser));
      break;
...
static void
blxo_helper_chooser_set_property (GObject      *object,
                                 guint         prop_id,
                                 const GValue *value,
                                 GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_CATEGORY:
      blxo_helper_chooser_set_category (chooser, g_value_get_enum (value));
      break;
...
static void
blxo_helper_chooser_update (BlxoHelperChooser *chooser)
{
  ...
  helper = blxo_helper_database_get_default (chooser->database, chooser->category);
  if (G_LIKELY (helper != NULL))
    {
      ...
      icon_name = blxo_helper_get_icon (helper);
      ...
      gtk_label_set_text (GTK_LABEL (chooser->label), blxo_helper_get_name (helper));
               ...
               BlxoHelperChooser *chooser)
{
  ...
  g_assert (BLXO_HELPER_N_CATEGORIES == G_N_ELEMENTS (CATEGORY_ERRORS));

  g_return_if_fail (GTK_IS_WIDGET (item));
  g_return_if_fail (BLXO_IS_HELPER_CHOOSER (chooser));

  /* determine the helper for the item */
  helper = g_object_get_data (G_OBJECT (item), I_("blxo-helper"));
  if (G_LIKELY (helper != NULL))
    {
      if (!blxo_helper_database_set_default (chooser->database, chooser->category, helper, &error))
        {
      ...
      else
        {
          ...
          blxo_helper_chooser_update (chooser);
        }
    }
}
                     ...
                     BlxoHelperChooser *chooser)
{
  ...
  g_assert (BLXO_HELPER_N_CATEGORIES == G_N_ELEMENTS (BROWSE_TITLES));
  g_assert (BLXO_HELPER_N_CATEGORIES == G_N_ELEMENTS (BROWSE_MESSAGES));
  ...
  helper = blxo_helper_database_get_custom (chooser->database, chooser->category);
  if (G_LIKELY (helper != NULL))
    {
      command = blxo_helper_get_command (helper);
  ...
  if (gtk_dialog_run (GTK_DIALOG (dialog)) == GTK_RESPONSE_OK)
    {
      ...
      blxo_helper_database_set_custom (chooser->database, chooser->category, command);
      ...
      if (G_LIKELY (helper != NULL))
        {
          ...
          g_object_set_data_full (G_OBJECT (item), I_("blxo-helper"), helper, g_object_unref);
...
static void
blxo_helper_chooser_pressed (BlxoHelperChooser *chooser,
                            GtkWidget        *button)
{
  AtkRelationSet *relations;
  AtkRelation    *relation;
  ...
  helpers = blxo_helper_database_get_all (chooser->database, chooser->category);
  for (lp = helpers, icon = NULL; lp != NULL; lp = lp->next)
    {
      /* determine the helper */
      helper = BLXO_HELPER (lp->data);

      /* create a menu item for the helper */
      item_label = gtk_label_new (blxo_helper_get_name (helper));
...
/**
 * blxo_helper_chooser_new:
 * @category : the #BlxoHelperCategory for the chooser.
 *
 * Allocates a new #BlxoHelperChooser for the given
 * @category.
 *
 * Return value: the newly allocated #BlxoHelperChooser.
 **/
GtkWidget*
blxo_helper_chooser_new (BlxoHelperCategory category)
{
  g_return_val_if_fail (category < BLXO_HELPER_N_CATEGORIES, NULL);
  return g_object_new (BLXO_TYPE_HELPER_CHOOSER, "category", category, NULL);
}



/**
 * blxo_helper_chooser_get_category:
 * @chooser : an #BlxoHelperChooser.
 *
 * Returns the #BlxoHelperCategory which is configured
 * by @chooser.
 *
 * Return value: the category for @chooser.
 **/
BlxoHelperCategory
blxo_helper_chooser_get_category (const BlxoHelperChooser *chooser)
{
  g_return_val_if_fail (BLXO_IS_HELPER_CHOOSER (chooser), BLXO_HELPER_WEBBROWSER);
...
/**
 * blxo_helper_chooser_set_category:
 * @chooser  : an #BlxoHelperChooser.
 ...
 **/
void
blxo_helper_chooser_set_category (BlxoHelperChooser *chooser,
                                 BlxoHelperCategory category)
{
  g_return_if_fail (BLXO_IS_HELPER_CHOOSER (chooser));
...
/**
 * blxo_helper_chooser_get_is_valid:
 * @chooser : an #BlxoHelperChooser.
 *
 ...
 **/
gboolean
blxo_helper_chooser_get_is_valid (const BlxoHelperChooser *chooser)
{
  g_return_val_if_fail (BLXO_IS_HELPER_CHOOSER (chooser), FALSE);
