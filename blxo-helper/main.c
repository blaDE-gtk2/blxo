...
static GtkWidget *
get_helper_error_dialog (BlxoHelperCategory  category,
                         GError            *error,
                         gboolean          *return_value)
{
  GtkWidget *dialog;
...
main (int argc, char **argv)
{
  ...
  g_assert (BLXO_HELPER_N_CATEGORIES == G_N_ELEMENTS (CATEGORY_EXEC_ERRORS));
   ...
   * FileManager and TerminalEmulator), since the blxo-helper utility will
  ...
  if (opt_configure == TRUE)
    {
      dialog = blxo_helper_chooser_dialog_new ();
      ...
      if (opt_socket_id != 0)
        {
          ...
          plug_child = blxo_helper_chooser_dialog_get_plug_child (BLXO_HELPER_CHOOSER_DIALOG (dialog));
  ...
  else if (opt_launch_type != NULL)
    {
      ...
      if (!blxo_helper_category_from_string (opt_launch_type, &category))
        {
      ...
      database = blxo_helper_database_get ();
      helper = blxo_helper_database_get_default (database, category);

      /* check if we have a valid helper */
      if (G_UNLIKELY (helper == NULL))
        {
          /* ask the user to choose a default helper for category */
          dialog = blxo_helper_launcher_dialog_new (category);
          if (startup_id != NULL)
            gtk_window_set_startup_id (GTK_WINDOW (dialog), startup_id);
          if (gtk_dialog_run (GTK_DIALOG (dialog)) == GTK_RESPONSE_OK)
            helper = blxo_helper_database_get_default (database, category);
          else
            blxo_helper_database_clear_default (database, category, NULL);
          gtk_widget_destroy (dialog);
      ...
      if (G_LIKELY (helper != NULL))
        {
          ...
          if (!blxo_helper_execute (helper, NULL, (argc > 1) ? argv[1] : NULL, &error))
            {
              if (!blxo_helper_database_get_dismissed (database, category))
                {
                  ...
                  if (dismissed)
                    {
                      blxo_helper_database_set_dismissed (database, category, dismissed);
                    }
                }
              g_error_free (error);
              result = EXIT_FAILURE;
    ...
    {
      /* try to parse the type */
      if (!blxo_helper_category_from_string (opt_query_type, &category))
        {
