 ...
 *
 * blxo-open --launch WebBrowser https://xfce.org (bug #5461).
 * blxo-open --launch WebBrowser https://xfce.org bugzilla.xfce.org 'http://www.google.com/search?q=what is a space' 'https://wiki.xfce.org'
 * blxo-open https://xfce.org
 * blxo-open --launch TerminalEmulator ./script.sh 'something with a space' 'nospace' (bug #5132).
 * blxo-open --launch TerminalEmulator ssh -l username some.host.com
 *
 * xfterm4 -e ssh -l username some.host.com (bug #5301, this generates line below)
 *   blxo-open --launch TerminalEmulator 'ssh -l username some.host.com'
 *
 * blxo-open /some/path/to/a/file.desktop
 * blxo-open somerelativefile
 **/
...
usage (void)
{
  g_print ("%s\n", _("Usage: blxo-open [URLs...]"));
  g_print ("%s\n", _("       blxo-open --launch TYPE [PARAMETERs...]"));
  g_print ("\n");
   ...
   * since the blxo-helper utility will not accept localized TYPEs.
   */
  ...
  g_print ("\n");
  g_print ("%s\n", _("If you don't specify the --launch option, blxo-open will open all specified\n"
...
static gboolean
blxo_open_launch_desktop_file (const gchar *arg)
{
#ifdef HAVE_GIO_UNIX
  GFile           *gfile;
  gchar           *contents;
...
static gchar *
blxo_open_get_path (const gchar *string)
{
  gchar *escaped;
  gchar *uri;
...
static gchar *
blxo_open_find_scheme (const gchar *string)
{
  gchar *current_dir;
  gchar *uri;
  gchar *path;
  ...
  if (g_path_is_absolute (string))
    return blxo_open_get_path (string);
   ...
   * blxo_str_looks_like_an_uri() test */
...
static gboolean
blxo_open_launch_category (const gchar *category,
                          const gchar *parameters)
{
  ...
  if (!blxo_execute_preferred_application (category, parameters, opt_working_directory, NULL, &error))
    {
...
static gboolean
blxo_open_uri_known_category (const gchar  *uri,
                             const gchar  *scheme,
                             gboolean     *succeed)
{
  guint        i;
      ...
      if (g_regex_match_simple (known_schemes[i].pattern, scheme, G_REGEX_CASELESS, 0))
        {
          ...
          *succeed = blxo_open_launch_category (known_schemes[i].category, uri);
              ...
              GError      **error)
{
  ...
  if (scheme != NULL && blxo_open_uri_known_category (uri, scheme, &retval))
    {
      g_free (scheme);
      return retval;
    }
      ...
      if (file_type == G_FILE_TYPE_DIRECTORY)
        {
          ...
          retval = blxo_open_launch_category ("FileManager", uri);
          succeed = TRUE;
        }
      else
        {
          ...
          for (i = 0; schemes[i] != NULL; i++)
            {
              ...
              if (strcmp (scheme, schemes[i]) == 0)
                {
                  retval = succeed = blxo_open_launch_category ("FileManager", uri);
                  break;
                }
            }
        }
  ...
  if (!succeed)
    {
      ...
      if (scheme != NULL
        ...
        retval = blxo_open_launch_category ("WebBrowser", uri);
      else
...
main (gint argc, gchar **argv)
{
  ...
  if (G_UNLIKELY (opt_help))
  ...
  else if (G_LIKELY (opt_launch != NULL))
    {
      ...
      /* run the preferred application */
      if (!blxo_open_launch_category (opt_launch, parameter))
        result = EXIT_FAILURE;
      ...
      for (argv += 1; result == EXIT_SUCCESS && *argv != NULL; ++argv)
        {
          if (g_str_has_suffix (*argv, ".desktop")
              && blxo_open_launch_desktop_file (*argv))
            {
              /* successfully launched a desktop file */
              continue;
            }
          else if (blxo_str_looks_like_an_uri (*argv))
            {
          ...
          else
            {
              ...
              uri = blxo_open_find_scheme (*argv);
            }
